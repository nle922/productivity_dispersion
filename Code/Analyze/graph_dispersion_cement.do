...
foreach letter in Q R{
	preserve
		...
		local collapsing_string = "(sd) sd = log_prod_`letter'"
		foreach percentile in 10 25 75 90 95 5{
			local collapsing_string = "`collapsing_string' (p`percentile') p`percentile' = log_prod_`letter' "
		}
		...
		twoway (line d_sd year, lwidth(thick)) (line d_diff9505 year, lwidth(thick) lpattern(dash)) (line d_diff9010 year, lwidth(thick) lpattern(dash))  (line d_diff7525 year, lwidth(thick) lpattern(dot)), xlab(1929(2)1935) ylab(, angle(45))  ytitle("Value Relative to 1929") legend(nobox region(lstyle(none))) xtitle("Year")
		graph export "Writing/Figures/Intensive/disp_cement_`letter'.pdf", replace
	restore
}
